...
GreatBomb::GreatBomb(){
	...
	this->timeRandom_ = std::uniform_int_distribution<int>(300,1000);
...
void GreatBomb::Draw(Vector3 pos){

	if(this->lights_.size() <= 0) return;

	SPRITE sp = Data.greatSp_;
...
void GreatBomb::ReadyBomb(){

	this->lights_.clear();
	this->time_ = 0;

	Light light;
	int instanceCounts = 5;
